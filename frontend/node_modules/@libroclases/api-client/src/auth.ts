...
export async function inviteStudent(req: InviteStudentRequest): Promise<UserDTO> {
  const { data } = await client.post<UserDTO>("/api/invite-student", req);
  return data;
}

export async function setPassword(req: SetPasswordRequest): Promise<LoginResponse> {
  const { data } = await client.post<LoginResponse>("/api/login/set-password", req);
  if (data.token) setToken(data.token);
  return data;
}
